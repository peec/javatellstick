...
public class Devices extends Panel {
	...
	public Devices(Shell shell) {
		...
		String[] titles = {"SwitchableDeviceImpl Name", "On/Off", "Dimmer", "Bell", "Learn", "Up", "Down", "Stop", "Execute"};
		...
		DeviceChangeListener listener = new DeviceChangeListener(){
			@Override
			public void onRequest(TellstickDeviceEvent event) {
		...
		final TellstickDeviceListener devlist = new TellstickDeviceListener(devices, listener);
				...
				public void widgetSelected(SelectionEvent e) {
					try {
						if ((JNA.CLibrary.TELLSTICK_TURNON & dev
								.getStatus()) > 0) {
							((SwitchableDeviceImpl) dev).off();
							button.setText("Turn on");
						} else {
							((SwitchableDeviceImpl) dev).on();
				...
				slider.addListener(SWT.Selection, new Listener() {
					public void handleEvent(Event event) {
						switch (event.detail) {
							...
							try {
								((DimmableDeviceImpl) dev).dim(val);
